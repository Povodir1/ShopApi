...
def serv_get_comments(item_id:int):
    with db_session() as session:
        ...
        return [CommentSchema(id =comment.id,username=comment.users.name,message=comment.message,rating = comment.rating,
...
def serv_patch_comment(item_id:int,user_id:int,new_data:CommentUpdateSchema):
    with db_session() as session:
        comment = session.query(Comment).filter(Comment.user_id ==user_id).filter(Comment.item_id == item_id).first()
        if not comment:
...
def serv_create_comment(data:CommentCreateSchema):
        with db_session() as session:
            is_available = session.query(Comment).filter(Comment.user_id == data.user_id).filter(Comment.item_id == data.item_id).first()
            if is_available:
