...
def get_comments(item_id:int,
                 ...
                 perm = Depends(check_permissions(Res.COMMENTS,Act.READ)),
...
async def patch_comments(item_id:int,
                         ...
                         perm = Depends(check_permissions(Res.COMMENTS,Act.UPDATE)),
                         session:Session = Depends(get_session)
                         ):
...
def delete_comments(item_id:int,
                    user: UserTokenDataSchema = Depends(get_token),
                    perm = Depends(check_permissions(Res.COMMENTS,Act.DELETE)),
                    session:Session = Depends(get_session)
                    ):
...
async def post_comments(item_id: int = Path(),
                        ...
                        media: Optional[list[UploadFile]] = File(None),
                        user: UserTokenDataSchema = Depends(get_token),
                        perm = Depends(check_permissions(Res.COMMENTS,Act.CREATE)),
                        session:Session = Depends(get_session)
                        ):
