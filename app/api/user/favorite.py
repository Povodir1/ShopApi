...
def get_favorite(user:UserTokenDataSchema = Depends(get_token),
                 perm = Depends(check_permissions(Res.FAVORITE_ITEMS,Act.READ)),
...
def add_to_favorite(item_id:int,
                    ...
                    perm = Depends(check_permissions(Res.FAVORITE_ITEMS,Act.CREATE)),
                    session:Session = Depends(get_session)
                    ):
...
def delete_from_favorite(item_id:int,
                         user:UserTokenDataSchema = Depends(get_token),
                         perm = Depends(check_permissions(Res.FAVORITE_ITEMS,Act.DELETE)),
                         session:Session = Depends(get_session)
                         ):
